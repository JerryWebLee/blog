...
function shouldInclude(filePath, isDirectory) {
  ...
  if (
    filePath.startsWith(".") &&
    ![".env", ".npmrc", ".prettierrc"].includes(filePath)
  ) {
    return false;
  }
...
async function collectDirectoryItems(dir, prefix = "") {
  try {
    ...
    for (const dirent of items) {
      ...
      const displayName =
        prefix + dirent.name + (dirent.isDirectory() ? "/" : "");
...
async function getScopes() {
  ...
  const [srcItems, rootItems] = await Promise.all([
    collectDirectoryItems(path.join(process.cwd(), "app"), "app/"),
  ...
  await fs.writeFile(
    configPath,
    JSON.stringify({ scopes: allScopes }, null, 2)
  );
...
// 使用示例;
getScopes()
  .then(() => console.log("----Generated Scopes Success----"))
  .catch(console.error);
